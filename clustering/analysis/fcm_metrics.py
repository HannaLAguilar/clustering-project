...
from clustering.cluster_algorithm.fuzzy_mean import FCM
...
def hyperparameter_clustering(X: np.ndarray,
                              ...
                              definitions.EXTERNAL_METRICS.values()) -> Tuple[List, List, float]:
    ...
    for param_comb in params_combs:
        ...
        t0 = time()
        # Perform clustering
        centers, labels = FCM(X, param['n_clusters'], param['m'], param['num_iters'], param['term_threshold'], param['limit'])
        tf = time() - t0
